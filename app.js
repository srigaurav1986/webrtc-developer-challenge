	...
	socket.on('checked-stream',function(data){
		...
		else
		{
			if(users[socket.key].length>2)
			{
				...
				io.sockets.in(socket.username).emit('full' );
				console.log("Sorry..This room is already in use..Try again later.");
				
			}
		}
	});
	socket.on('end-meeting-room',function(data){
		console.log('End Meeting ...');
		socket.broadcast.to(socket.key).emit('meeting-is-ending');
		removeRoomKey(socket.key);
		});
...
//FUNCTIONS----start
	function addUserKey(data,socket)
	{
		try
		{
				...
				if(rooms.indexOf(key) === -1)
			{
				...
				console.log("Room Name is",socket.key);
			...
			else
			{
				if(users[key]!=undefined)
				{
					...
					console.log("Total number of participants in room", users[key],"  ",users[key].length);
					if (users[key].length >2 )
					{
						socket.emit('full', key);
					}
					else
					{
					socket.users = {};
	...
	function removeRoomKey(socket)
	{
		rooms.splice(socket.key,1);
