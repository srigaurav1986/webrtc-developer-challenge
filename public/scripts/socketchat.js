...
$(window).load(function(){
	...
	socket.on('meeting-is-ending',function(data){
...
function getUser() {
  ...
  navigator.mediaDevices.getUserMedia(constraints).then(success).then(gotDevices).catch(handleError);
}

audioInputSelect.onchange = getUser;
audioOutputSelect.onchange = changeAudioDestination;

videoSelect.onchange = getUser;
...
function addRemoteVideo(top, height, stream)
{
	if(!socket.screenSharedByRemote && !socket.screenShared)
	{
		...
		$('#local-video').find('video')[0].srcObject=stream.stream;
...
//WEBRTC FUNCTIONS --- START
	...
	function gotLocalVideo(stream)
	{
		...
		//NEW CODE//
		  ...
		  } else {objstream = stream.stream;
		    
		  }
		//NEW CODE//
		addRemoteVideo(x,y,stream);
		if(socket.screenSharedByRemote)
		{
	...
	function createAnswer(localStream, sdp)
	{
		...
		var TURN = {
		    url: 'turn:gaurav@52.66.104.148:3478',
		    credential: 'gaurav'
		};
